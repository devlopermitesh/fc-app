...
const bubbleStyles: Record<RoomMessage["author"], string> = {
  you: "ml-auto border-gold/22 bg-[linear-gradient(180deg,rgba(246,196,0,0.22),rgba(147,92,8,0.34))] text-[#fff4d8]",
  match:
    "mr-auto border-white/10 bg-[linear-gradient(180deg,rgba(48,48,48,0.92),rgba(25,25,25,0.92))] text-white/82",
  system:
    "mx-auto border-transparent bg-transparent px-0 py-0 text-[11px] uppercase tracking-[0.28em] text-white/24 shadow-none",
};
    ...
    <div className="relative flex-1 overflow-y-auto px-3 pb-36 pt-4 sm:px-4 sm:pt-5">
      ...
      <div className="mx-auto flex w-full max-w-3xl flex-col gap-3">
        {messages.map((message, index) => {
          ...
          const isGold = message.emphasis === "gold";
          ...
          return (
            <motion.div
              ...
              className={`max-w-[88%] rounded-[1.45rem] border px-4 py-3 shadow-[0_18px_35px_rgba(0,0,0,0.18)] sm:max-w-[72%] ${bubbleStyles[message.author]} ${isSystem ? "max-w-full" : ""}`}
            >
              {isSystem ? (
